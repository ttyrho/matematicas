...
:multiline-flashcard:
Si $A$ es hermítica entonces $A=A^*$ donde $A^*$ es la matriz traspuesta conjugada de $A$. Si $a_{kk} = a_k + ib_k$, entonces la entrada $(k,k)$ de $A^*$ es $\overline{a_{kk}} = a_k - ib_k$. Luego $$a_{kk} = \overline{a_{kk}} \Rightarrow a_k+ib_k = a_k-ib_k \Rightarrow 2ib_k=0 \Rightarrow b_k=0 \hspace{2em} \square$$
